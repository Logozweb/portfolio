...
app.post('/api/works/reorder', requireAuth, (req, res) => {
  ...
  try {
    const updateStmt = db.prepare('UPDATE works SET sort_order = ? WHERE id = ? AND category = ?');
    const updateMany = db.transaction((cleanIds) => {
      cleanIds.forEach((id, idx) => updateStmt.run(idx, id, category));
    });
    updateMany(cleanIds);
    const selectStmt = db.prepare('SELECT id, sort_order FROM works WHERE category = ? ORDER BY sort_order ASC');
    const rows = selectStmt.all(category);
...
app.post('/api/works/:id/edit', requireAuth, upload.single('image'), (req, res) => {
  ...
  try {
    ...
    const maxStmt = db.prepare('SELECT COALESCE(MAX(sort_order), -1) + 1 as next FROM works WHERE category = ?');
    const row = maxStmt.get(category);
    const nextOrder = (row && typeof row.next === 'number') ? row.next : 0;
    ...
    const params = updateWithImage ? [title, category, live_url || null, imgRel, nextOrder, id] : [title, category, live_url || null, nextOrder, id];
    updateStmt.run(...params);

    const selectStmt = db.prepare('SELECT * FROM works WHERE id = ?');
    const work = selectStmt.get(id);
...
app.post('/api/change-credentials', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const updateStmt = db.prepare('UPDATE users SET username = ?, password_hash = ?, email = ? WHERE id = ?');
    updateStmt.run(new_username, hash, new_email, req.session.userId);
    res.json({ ok: true });
  } catch (e) {
    const msg = String(e);
